...
describe('GET /todos/:id', () => {
  ...
  it('should NOT get specific instance of todo model created by another user', (done) => {
    ...
    request(app)
      ...
      .expect(404)
      .expect((response) => {
        expect(response.body.todo).toBeUndefined();
      })
      .end(done);
  });
...
describe('DELETE /todos/:id', () => {
  ...
  it('should NOT remove a todo if user not authenticated', (done) => {
    let id = todos[1]._id.toHexString();
    request(app)
      .delete(`/todos/${id}`)
      .set('x-auth', users[0].tokens[0].token)
...
describe('PATCH /todos/:id', () => {
  ...
  it('should not update todo if user not authenticated', (done) => {
    let id = todos[0]._id.toHexString();
    let text = 'this should be the new text';
    request(app)
      .patch(`/todos/${id}`)
